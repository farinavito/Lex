...
def test_wasContractBreached_timeNotBreached_false_status_Terminated(deploy):
    '''check if the wasContractBreached function when timeNotBreached is false (agreement's duration was breached), changes status to Terminated'''
    #Activated
    deploy.sendPayment(agreements_number, {'from': accounts[sender], 'value': amount_sent})
    chain = Chain()
    chain.sleep(2700000)
...
def test_wasContractBreached_timeNotBreached_false_status_Terminated_2(deploy, loops):
    '''check if the wasContractBreached function when timeNotBreached is false (agreement's duration was breached), changes status to Terminated'''
    #Activated
...
@pytest.mark.parametrize("loops", [3, 4, 5, 6])
def test_wasContractBreached_timeNotBreached_false_send_deposit_2(deploy, loops):
    '''check if the wasContractBreached function when timeNotBreached is false (agreement's duration was breached), sends a deposit to the receiver'''
    #Activated
    balance_receiver = accounts[receiver].balance()
    for _ in range(2, loops):
        deploy.sendPayment(agreements_number, {'from': accounts[sender], 'value': amount_sent})
...
def test_wasContractBreached_timeNotBreached_true_totalDepositSent(deploy):
    '''check if the totalDepositSent() is incremented by the deposit (agreement's duration was breached)'''
...
@pytest.mark.parametrize("loops", [3, 4, 5, 6])
def test_wasContractBreached_timeNotBreached_true_totalDepositSent_2(deploy, loops):
    '''check if the totalDepositSent() is incremented by the deposit (agreement's duration was breached)'''
    totalDepositBefore = deploy.totalDepositSent()
    #Activated
    for _ in range(2, loops):
...
@pytest.mark.parametrize("loops", [2, 3, 4, 5])
@pytest.mark.parametrize("time", [10, 100, 1000, 15000])
def test_wasContractBreached_timeNotBreached_true_totalDepositSent_3(deploy, loops, time):
...
@pytest.mark.parametrize("loops", [2, 3, 4, 5])
@pytest.mark.parametrize("time", [10, 100, 1000, 15000])
def test_wasContractBreached_timeNotBreached_false_status_deposit_equals_zero_3(deploy, loops, time):
    '''check if the wasContractBreached function when timeNotBreached is false (agreement's every period was breached), changes deposit to 0'''
    #Activated
    for _ in range(1,loops):
        deploy.sendPayment(agreements_number, {'from': accounts[sender], 'value': amount_sent})
        chain = Chain()
...
def test_getMyNumAgreementsSender_success(deploy):
    ...
    now = chain.time()
    startAgreement = now + 1
    deploy.createAgreement(accounts[1], amount_sent, every_period, agreement_duration, startAgreement, {'from': accounts[2], 'value': amount_sent})
