...
def test_exactAgreement_signee(deploy):
    ...
    assert deploy.exactAgreement(0)[1] == accounts[signee]
...
def test_exactAgreement_receiver(deploy):
    ...
    assert deploy.exactAgreement(0)[2] == accounts[receiver]

def test_exactAgreement_amount(deploy):
    '''check if the amount of the agreement is 2'''
    assert deploy.exactAgreement(0)[3] == amount_sent  
...
def test_event_AgreementInfo_agreementId(new_agreement_7, deploy):
    ...
    assert new_agreement_7.events[0]["agreementId"] == deploy.exactAgreement(0)[0]
...
def test_event_AgreementInfo_agreementSignee(new_agreement_7, deploy):
    ...
    assert new_agreement_7.events[0]["agreementSignee"] == deploy.exactAgreement(0)[1]
...
def test_event_AgreementInfo_agreementReceiver(new_agreement_7, deploy):
    ...
    assert new_agreement_7.events[0]["agreementReceiver"] == deploy.exactAgreement(0)[2]
...
def test_event_AgreementInfo_agreementAmount(new_agreement_7, deploy):
    ...
    assert new_agreement_7.events[0]["agreementAmount"] == deploy.exactAgreement(0)[3]

def test_event_AgreementInfo_transactionCreated(new_agreement_7, deploy):
    '''check if the event AgreementInfo emits correctly agreementAmount'''
    assert new_agreement_7.events[0]["agreementTransactionCreated"] == deploy.exactAgreement(0)[4]

def test_event_AgreementInfo_agreementDeposit(new_agreement_7, deploy):
    '''check if the event AgreementInfo emits correctly agreementAmount'''
    assert new_agreement_7.events[0]["agreementDeposit"] == deploy.exactAgreement(0)[5]
...
def test_event_AgreementInfo_agreementStatus(new_agreement_7, deploy):
    ...
    assert new_agreement_7.events[0]["agreementStatus"] == deploy.exactAgreement(0)[6]

def test_event_AgreementInfo_agreementApproved(new_agreement_7, deploy):
    '''check if the event AgreementInfo emits correctly agreementStatus'''
    assert new_agreement_7.events[0]["agreementApproved"] == deploy.exactAgreement(0)[7]
...
def test_event_AgreementInfo_agreementTimeCreation(new_agreement_7, deploy):
    ...
    assert new_agreement_7.events[0]["agreementTimeCreation"] == deploy.exactAgreement(0)[8]
...
def test_event_AgreementInfo_agreementPositionPeriod(new_agreement_7, deploy):
    ...
    assert new_agreement_7.events[0]["agreementPositionPeriod"] == deploy.exactAgreement(0)[10]
...
def test_MySenderAgreements_emits_correctly_agreementId_agreements_1(deploy):
    ...
    assert deploy.MySenderAgreements(accounts[signee], {'from': accounts[signee]}).events[0]['agreementId'] == deploy.exactAgreement(0)[0]   
...
def test_MySenderAgreements_emits_correctly_agreementSignee_agreements_1(deploy):
    ...
    assert deploy.MySenderAgreements(accounts[signee], {'from': accounts[signee]}).events[0]['agreementSignee'] == deploy.exactAgreement(0)[1]
...
def test_MySenderAgreements_emits_correctly_agreementReceiver_agreements_1(deploy):
    ...
    assert deploy.MySenderAgreements(accounts[signee], {'from': accounts[signee]}).events[0]['agreementReceiver'] == deploy.exactAgreement(0)[2]
...
def test_MySenderAgreements_emits_correctly_agreementAmount_agreements_1(deploy):
    ...
    assert deploy.MySenderAgreements(accounts[signee], {'from': accounts[signee]}).events[0]['agreementAmount'] == deploy.exactAgreement(0)[3]
...
def test_MySenderAgreements_emits_correctly_agreementTransactionCreated(deploy):
    ...
    assert deploy.MySenderAgreements(accounts[signee], {'from': accounts[signee]}).events[0]['agreementTransactionCreated'] == deploy.exactAgreement(0)[4]
...
def test_MySenderAgreements_emits_correctly_agreementDeposit_agreements_1(deploy):
    ...
    assert deploy.MySenderAgreements(accounts[signee], {'from': accounts[signee]}).events[0]['agreementDeposit'] == deploy.exactAgreement(0)[5]
...
def test_MySenderAgreements_emits_correctly_agreementStatus_agreements_1(deploy):
    ...
    assert deploy.MySenderAgreements(accounts[signee], {'from': accounts[signee]}).events[0]['agreementStatus'] == deploy.exactAgreement(0)[6]
...
def test_MySenderAgreements_emits_correctly_agreementApproved_agreements_1(deploy):
    ...
    assert deploy.MySenderAgreements(accounts[signee], {'from': accounts[signee]}).events[0]['agreementApproved'] == deploy.exactAgreement(0)[7]
...
def test_MySenderAgreements_emits_correctly_agreementTimeCreation_agreements_1(deploy):
    ...
    assert deploy.MySenderAgreements(accounts[signee], {'from': accounts[signee]}).events[0]['agreementTimeCreation'] == deploy.exactAgreement(0)[8]
...
def test_MySenderAgreements_emits_correctly_agreementTimePeriods_agreements_1(deploy):
    ...
    assert deploy.MySenderAgreements(accounts[signee], {'from': accounts[signee]}).events[0]['agreementTimePeriods'] == deploy.exactAgreement(0)[9]
...
def test_MySenderAgreements_emits_correctly_agreementPositionPeriod_agreements_1(deploy):
    ...
    assert deploy.MySenderAgreements(accounts[signee], {'from': accounts[signee]}).events[0]['agreementPositionPeriod'] == deploy.exactAgreement(0)[10]
...
def test_MySenderAgreements_emits_correctly_agreementTimeDuration_agreements_1(deploy):
    ...
    assert deploy.MySenderAgreements(accounts[signee], {'from': accounts[signee]}).events[0]['agreementTimeDuration'] == deploy.exactAgreement(0)[11]
...
def test_MyReceiverAgreements_emits_correctly_agreementId_agreements_1(deploy):
    ...
    assert deploy.MyReceiverAgreements(accounts[receiver], {'from': accounts[receiver]}).events[0]['agreementId'] == deploy.exactAgreement(0)[0]
...
def test_MyReceiverAgreements_emits_correctly_agreementSignee_agreements_1(deploy):
    ...
    assert deploy.MyReceiverAgreements(accounts[receiver], {'from': accounts[receiver]}).events[0]['agreementSignee'] == deploy.exactAgreement(0)[1]
...
def test_MyReceiverAgreements_emits_correctly_agreementReceiver_agreements_1(deploy):
    ...
    assert deploy.MyReceiverAgreements(accounts[receiver], {'from': accounts[receiver]}).events[0]['agreementReceiver'] == deploy.exactAgreement(0)[2]
...
def test_MyReceiverAgreements_emits_correctly_agreementAmount_agreements_1(deploy):
    ...
    assert deploy.MyReceiverAgreements(accounts[receiver], {'from': accounts[receiver]}).events[0]['agreementAmount'] == deploy.exactAgreement(0)[3]
...
def test_MyReceiverAgreements_emits_correctly_agreementTransactionCreated_agreements_1(deploy):
    ...
    assert deploy.MyReceiverAgreements(accounts[receiver], {'from': accounts[receiver]}).events[0]['agreementTransactionCreated'] == deploy.exactAgreement(0)[4]
...
def test_MyReceiverAgreements_emits_correctly_agreementDeposit_agreements_1(deploy):
    ...
    assert deploy.MyReceiverAgreements(accounts[receiver], {'from': accounts[receiver]}).events[0]['agreementDeposit'] == deploy.exactAgreement(0)[5]
...
def test_MyReceiverAgreements_emits_correctly_agreementStatus_agreements_1(deploy):
    ...
    assert deploy.MyReceiverAgreements(accounts[receiver], {'from': accounts[receiver]}).events[0]['agreementStatus'] == deploy.exactAgreement(0)[6]
...
def test_MyReceiverAgreements_emits_correctly_agreementApproved_agreements_1(deploy):
    ...
    assert deploy.MyReceiverAgreements(accounts[receiver], {'from': accounts[receiver]}).events[0]['agreementApproved'] == deploy.exactAgreement(0)[7]
...
def test_MyReceiverAgreements_emits_correctly_agreementTimeCreation_agreements_1(deploy):
    ...
    assert deploy.MyReceiverAgreements(accounts[receiver], {'from': accounts[receiver]}).events[0]['agreementTimeCreation'] == deploy.exactAgreement(0)[8]
...
def test_MyReceiverAgreements_emits_correctly_agreementTimePeriods_agreements_1(deploy):
    ...
    assert deploy.MyReceiverAgreements(accounts[receiver], {'from': accounts[receiver]}).events[0]['agreementTimePeriods'] == deploy.exactAgreement(0)[9]
...
def test_MyReceiverAgreements_emits_correctly_agreementPositionPeriod_agreements_1(deploy):
    ...
    assert deploy.MyReceiverAgreements(accounts[receiver], {'from': accounts[receiver]}).events[0]['agreementPositionPeriod'] == deploy.exactAgreement(0)[10]
...
def test_MyReceiverAgreements_emits_correctly_agreementTimeDuration_agreements_1(deploy):
    ...
    assert deploy.MyReceiverAgreements(accounts[receiver], {'from': accounts[receiver]}).events[0]['agreementTimeDuration'] == deploy.exactAgreement(0)[11]
...
def test_ConfirmAgreement_agreement_already_confirmed(deploy):
    ...
    function_enabled = deploy.ConfirmAgreement(0, {'from': accounts[receiver]})
...
def test_ConfirmAgreement_fail_require_1(deploy, accounts_number):
    ...
    try:
        deploy.ConfirmAgreement(0, {'from': accounts[accounts_number]})        
    except Exception as e:
...
def test_terminateContract_emit_Terminated_initial_status_activated_already_terminated(deploy):
    ...
    function_enabled = deploy.terminateContract(0, {'from': accounts[signee]})
    message = function_enabled.events[0][0]['message']
...
def test_sendPayments_fails_require_wrong_address(deploy, accounts_number):
    ...
    try:
        ...
        deploy.sendPayment(0, {'from': accounts[accounts_number], 'value': amount_sent})
    except Exception as e:
...
def test_sendPayments_change_agreementDeposit_initial_status_created(deploy):
    ...
    deploy.ConfirmAgreement(0, {'from': accounts[receiver]})
    deploy.sendPayment(0, {'from': accounts[signee], 'value': amount_sent})
    assert deploy.exactAgreement(0)[5] == amount_sent
...
def test_sendPayments_emit_NotifyUser_initial_status_created(deploy):
    ...
    deploy.ConfirmAgreement(0, {'from': accounts[receiver]})
    function_enabled = deploy.sendPayment(0, {'from': accounts[signee], 'value': amount_sent})
    message = function_enabled.events[0][0]['message']
...
def test_timeNotBreached_value_smaller_amount_emit_Terminated(deploy, value_sent):
    ...
    deploy.sendPayment(0, {'from': accounts[signee], 'value': amount_sent})
    #the contract has been activated, now send the smaller quantity of money again
    function_initialize = deploy.sendPayment(0, {'from': accounts[signee], 'value': value_sent})
